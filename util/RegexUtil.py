...
class RegexUtil(object):
    ...
    def getTokenTypes(self, token, posTag):
        """
        Get token types for specific token with specific pos-tag.
        ...
        if self.YEAR_PATTERN_1.fullmatch(token) or self.YEAR_PATTERN_2.fullmatch(token) \
                or self.YEAR_MID_PATTERN.fullmatch(token) or self.ERA_YEAR_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.YEAR)

        mYearYear = self.YEAR_YEAR_PATTERN.fullmatch(token)
        if mYearYear:
            tokenTypes.add(TokenType.YEAR_YEAR)

        if isPosTagNN and self.SEASON_PATTERN.fullmatch(token) or self.SEASON_MID_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.SEASON)

        if isPosTagNN and self.MONTH_PATTERN.fullmatch(token) or self.MONTH_MID_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.MONTH)
        if isPosTagNN and self.MONTH_ABBR_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.MONTH_ABBR)
        if self.MONTH_MONTH_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.MONTH_MONTH)

        if self.YEAR_MONTH_PATTERN_1.fullmatch(token) or self.YEAR_MONTH_PATTERN_2.fullmatch(token):
            tokenTypes.add(TokenType.YEAR_MONTH)

        if isPosTagNN and self.WEEK_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.WEEK)

        if isPosTagNN and self.WEEK_ABBR_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.WEEK_ABBR)

        if self.WEEK_WEEK_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.WEEK_WEEK)

        if self.DATE_PATTERN_1.fullmatch(token) or self.DATE_PATTERN_2.fullmatch(token) or self.DATE_PATTERN_3.fullmatch(token):
            tokenTypes.add(TokenType.DATE)

        if self.TIME_PATTERN_1.fullmatch(token) or self.TIME_PATTERN_2.fullmatch(token):
            tokenTypes.add(TokenType.TIME)
        if self.TIME_TIME_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.TIME_TIME)

        if self.HALFDAY_PATTERN_1.fullmatch(token) or self.HALFDAY_PATTERN_2.fullmatch(token):
            tokenTypes.add(TokenType.HALFDAY)
        if self.HALFDAY_HALFDAY_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.HALFDAY_HALFDAY)

        if self.TIME_ZONE_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.TIME_ZONE)

        if isPosTagNN and self.ERA_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.ERA)

        if isPosTagNN and self.TIME_UNIT_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.TIME_UNIT)

        mDuration = self.DURATION_PATTERN.fullmatch(token)
        if (isPosTagNN or isPosTagJJ or isPosTagCD) and mDuration:
            tokenTypes.add(TokenType.DURATION)

        if not mDuration and (
                self.DURATION_DURATION_PATTERN_1.fullmatch(token) or self.DURATION_DURATION_PATTERN_2.fullmatch(token)):
            tokenTypes.add(TokenType.DURATION_DURATION)

        if (isPosTagNN or isPosTagRB or isPosTagJJ or self.DAY_TIME_MID_PATTERN.fullmatch(token)) and \
                self.DAY_TIME_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.DAY_TIME)

        if (isPosTagNN or isPosTagRB) and self.TIMELINE_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.TIMELINE)

        if self.HOLIDAY_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.HOLIDAY)

        if (isPosTagNN or isPosTagRB or isPosTagJJ) and self.PERIOD_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.PERIOD)

        if isPosTagNN and self.DECADE_PATTERN.fullmatch(token) or self.DECADE_MID_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.DECADE)

        mBasicNum2 = self.BASIC_NUMBER_PATTERN_2.fullmatch(token)
        if ((isPosTagJJ or isPosTagCD or isPosTagRB) and (
                self.ORDINAL_PATTERN_1.fullmatch(token) or self.ORDINAL_PATTERN_2.fullmatch(token))) \
                or self.DIGIT_PATTERN_1.fullmatch(token) or self.DIGIT_PATTERN_2.fullmatch(token) \
                or self.BASIC_NUMBER_PATTERN_1.fullmatch(token) or mBasicNum2:
            tokenTypes.add(TokenType.NUMERAL)

        if not mYearYear and not mBasicNum2 and (
                self.DIGIT_DIGIT_PATTERN.fullmatch(token) or self.BASIC_NUMBER_NUMBER_PATTERN.fullmatch(token)
                or self.ORDINAL_ORDINAL_PATTERN.fullmatch(token)):
            tokenTypes.add(TokenType.NUMERAL_NUMERAL)

        if self.INARTICLE_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.INARTICLE)

        if len(tokenTypes) == 0 and (not isPosTagNN and self.PREFIX_PATTERN_1.fullmatch(token) or
                                     isPosTagNN and self.PREFIX_PATTERN_2.fullmatch(token)):
            tokenTypes.add(TokenType.PREFIX)

        if self.SUFFIX_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.SUFFIX)

        if self.LINKAGE_PATTERN.fullmatch(token):
            tokenTypes.add(TokenType.LINKAGE)

        if self.COMMA_PATTERN.fullmatch(token):
